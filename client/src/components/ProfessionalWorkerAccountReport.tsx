  ...
  return (
    ...
    >
      ...
      <div style={{marginBottom: '5mm'}}>
        <h2 style={{fontSize: '24px', fontWeight: 'bold', color: '#1e40af', marginBottom: '6px', textAlign: 'center', background: 'linear-gradient(135deg, #dbeafe, #bfdbfe)', padding: '8px', borderRadius: '8px', border: '2px solid #1e40af'}}>سجل الحضور والأجور التفصيلي</h2>
        <table className="enhanced-summary-table" style={{width: '100%', borderCollapse: 'collapse', fontSize: '18px', lineHeight: '1.4', border: '3px solid #1e40af', borderRadius: '8px', overflow: 'hidden'}}>
          <thead>
            <tr className="enhanced-header-row preserve-color" style={{background: 'linear-gradient(135deg, #1e40af, #3b82f6)', color: '#ffffff', height: '75px'}}>
              <th style={{padding: '12px 8px', textAlign: 'center', fontWeight: 'bold', border: '2px solid #1e3a8a', width: '8%', fontSize: '20px'}}>م</th>
              <th style={{padding: '12px 12px', textAlign: 'center', fontWeight: 'bold', border: '2px solid #1e3a8a', width: '15%', fontSize: '20px'}}>التاريخ الميلادي</th>
              <th style={{padding: '12px 15px', textAlign: 'right', fontWeight: 'bold', border: '2px solid #1e3a8a', width: '27%', fontSize: '20px'}}>وصف العمل المنجز والمهام اليومية</th>
              <th style={{padding: '12px 8px', textAlign: 'center', fontWeight: 'bold', border: '2px solid #1e3a8a', width: '12%', fontSize: '20px'}}>ساعات العمل</th>
              <th style={{padding: '12px 12px', textAlign: 'center', fontWeight: 'bold', border: '2px solid #1e3a8a', width: '18%', fontSize: '20px'}}>الأجر اليومي المستحق</th>
              <th style={{padding: '12px 12px', textAlign: 'center', fontWeight: 'bold', border: '2px solid #1e3a8a', width: '20%', fontSize: '20px'}}>المبلغ المدفوع فعلياً</th>
            </tr>
          </thead>
          <tbody>
            {attendance.map((record: any, index: number) => (
              <tr key={record.id || index} style={{background: index % 2 === 0 ? '#f8fafc' : 'white', minHeight: '65px', borderBottom: '1px solid #cbd5e1'}}>
                <td style={{padding: '12px 8px', textAlign: 'center', border: '1px solid #cbd5e1', fontWeight: 'bold', fontSize: '18px', color: '#475569'}}>{index + 1}</td>
                <td style={{padding: '12px 8px', textAlign: 'center', border: '1px solid #cbd5e1', fontSize: '17px', color: '#1e293b', fontWeight: '600'}}>{formatDate(record.date)}</td>
                <td style={{padding: '12px 12px', textAlign: 'right', border: '1px solid #cbd5e1', fontSize: '17px', color: '#374151', lineHeight: '1.4'}}>{record.workDescription || 'عمل يومي'}</td>
                <td style={{padding: '12px 8px', textAlign: 'center', border: '1px solid #cbd5e1', fontSize: '17px', color: '#6b7280', fontWeight: 'bold'}}>
                  {record.startTime && record.endTime ? `${record.startTime} - ${record.endTime}` : '8 ساعات'}
                </td>
                <td style={{padding: '12px 12px', textAlign: 'center', border: '1px solid #cbd5e1', fontSize: '18px', color: '#059669', fontWeight: 'bold', background: '#f0fdf4'}}>{formatCurrency(Number(record.dailyWage) || 0)}</td>
                <td style={{padding: '12px 12px', textAlign: 'center', border: '1px solid #cbd5e1', fontSize: '18px', color: '#0d9488', fontWeight: 'bold', background: '#f0fdfa'}}>{formatCurrency(Number(record.paidAmount) || 0)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr style={{background: 'linear-gradient(135deg, #10b981, #059669)', color: 'white', height: '65px'}}>
              <td colSpan={4} style={{padding: '12px', textAlign: 'center', border: '2px solid #047857', fontWeight: 'bold', fontSize: '20px'}}>إجمالي الأجور والمدفوعات</td>
              <td style={{padding: '12px', textAlign: 'center', border: '2px solid #047857', fontWeight: 'bold', fontSize: '20px'}}>{formatCurrency(totalEarned)}</td>
              <td style={{padding: '12px', textAlign: 'center', border: '2px solid #047857', fontWeight: 'bold', fontSize: '20px'}}>{formatCurrency(totalPaid)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div style={{background: 'linear-gradient(135deg, #fef3c7, #fde68a)', padding: '15px', borderRadius: '12px', border: '3px solid #f59e0b', marginTop: '4mm'}}>
        <h2 style={{fontSize: '26px', fontWeight: 'bold', color: '#92400e', textAlign: 'center', marginBottom: '12px'}}>الملخص النهائي والرصيد الإجمالي</h2>
        <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', fontSize: '20px'}}>
          <div style={{background: '#ecfdf5', padding: '12px', borderRadius: '8px', textAlign: 'center', border: '2px solid #10b981'}}>
            <div style={{fontSize: '18px', color: '#047857', marginBottom: '6px'}}>إجمالي المكتسب</div>
            <div style={{fontSize: '24px', fontWeight: 'bold', color: '#059669'}}>{formatCurrency(totalEarned)}</div>
          </div>
          <div style={{background: '#f0fdfa', padding: '12px', borderRadius: '8px', textAlign: 'center', border: '2px solid #0d9488'}}>
            <div style={{fontSize: '18px', color: '#0f766e', marginBottom: '6px'}}>إجمالي المدفوع</div>
            <div style={{fontSize: '24px', fontWeight: 'bold', color: '#0d9488'}}>{formatCurrency(totalPaid)}</div>
          </div>
          <div style={{background: '#fef2f2', padding: '12px', borderRadius: '8px', textAlign: 'center', border: '2px solid #dc2626'}}>
            <div style={{fontSize: '18px', color: '#991b1b', marginBottom: '6px'}}>إجمالي المحول</div>
            <div style={{fontSize: '24px', fontWeight: 'bold', color: '#dc2626'}}>{formatCurrency(totalTransferred)}</div>
          </div>
        </div>
        <div style={{marginTop: '15px', padding: '15px', background: currentBalance >= 0 ? '#ecfdf5' : '#fef2f2', borderRadius: '10px', textAlign: 'center', border: `3px solid ${currentBalance >= 0 ? '#10b981' : '#dc2626'}`}}>
          <div style={{fontSize: '22px', color: currentBalance >= 0 ? '#047857' : '#991b1b', marginBottom: '8px'}}>الرصيد النهائي للعامل</div>
          <div style={{fontSize: '36px', fontWeight: 'bold', color: currentBalance >= 0 ? '#059669' : '#dc2626'}}>{formatCurrency(currentBalance)}</div>
          <div style={{fontSize: '16px', color: '#6b7280', marginTop: '6px', fontStyle: 'italic'}}>
      ...
      {/* Report Footer */}
      <div style={{marginTop: '8mm', padding: '8px', borderTop: '2px solid #9ca3af', textAlign: 'center', fontSize: '14px', color: '#6b7280'}}>
        <p>تم إنشاء هذا التقرير بتاريخ {formatDate(new Date().toISOString().split('T')[0])} | نظام إدارة المشاريع الإنشائية</p>
        <p style={{marginTop: '4px'}}>كشف حساب العامل - {worker.name} | المشروع: {selectedProject?.name}</p>
      </div>
    </div>
  );
};